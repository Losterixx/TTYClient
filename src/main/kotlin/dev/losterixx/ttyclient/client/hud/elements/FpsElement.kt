...
class FpsElement(private val config: () -> FpsHudConfig) : HudElement() {
    ...
    override fun resolveText(): String =
        config().text.replace("{FPS}", Minecraft.getInstance().fps.toString(), ignoreCase = true)

    override fun previewText(): String =
        config().text.replace("{FPS}", "60", ignoreCase = true)
